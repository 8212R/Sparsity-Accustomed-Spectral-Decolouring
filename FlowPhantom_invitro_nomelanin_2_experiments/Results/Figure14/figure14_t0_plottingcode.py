...
plot_pixel_coords = [[entry[0]-136,entry[1]-142] for entry in chosen_pixels]
...
plotdata = np.reshape(data[0][7],(333,333))
clipped_plotdata = plotdata.copy()[142:202,136:196]
fig,ax = plt.subplots()
plt.imshow(clipped_plotdata, origin = 'lower')
ax.xaxis.set_visible(False)
ax.yaxis.set_visible(False)
circle = plt.Circle((30,30),8.0, fill = False, edgecolor= 'r', linewidth=5.0)
ax.add_patch(circle)
clb = plt.colorbar()
file = 'T=0_PAimage_NOMELANIN.png'
plt.savefig(file)
plt.show()

# SASD image at t = 0
a = np.load('Time0_SASD_predictions.npy')*100
print(np.mean(a))
...
plt.imshow(np.float32(SASD_t0_plotdata),interpolation = 'nearest', vmin = np.mean(a) - 8, vmax = np.mean(a)+3, origin = 'lower')
clb = plt.colorbar()
...
file = 'T=0_SASD_NOMELANIN.png'
plt.savefig(file)
plt.show()


# LSD image at t = 0
b = np.load('Time0_LSD_predictions.npy')*100
...
plt.imshow(np.float32(LSD_t0_plotdata),interpolation = 'nearest', vmin = np.mean(b)-8, vmax = np.mean(b)+3, origin = 'lower')
clb = plt.colorbar()
clb.ax.set_title('sO$_2$ [%]')
circle = plt.Circle((30,30),8.0, fill = False, edgecolor= 'r', linewidth=5.0)
...
file = 'T=0_LSD_NOMELANIN.png'
plt.savefig(file)
plt.show()


# LU image at t = 0
c = np.load('Time0_LU_predictions.npy')*100
...
plt.imshow(np.float32(LU_t0_plotdata),interpolation = 'nearest', vmin = np.mean(c)-8, vmax = np.mean(c)+3, origin = 'lower')
clb = plt.colorbar()
clb.ax.set_title('sO$_2$ [%]')
circle = plt.Circle((30,30),8.0, fill = False, edgecolor= 'r', linewidth=5.0)

...
for i in range(timesteps):
    ...
    if i ==0:
        np.save('Time0_LSD_predictions.npy', np.array(invitro_predictions.cpu().detach().numpy()))
    if i ==200:
        np.save('Time200_LSD_predictions.npy', np.array(invitro_predictions.cpu().detach().numpy()))
    timestep_so2 = torch.mean(invitro_predictions)
    timestep_stddev = torch.std(invitro_predictions)
    so2_predictions.append(timestep_so2.cpu().detach().numpy())
    so2_stddevs.append(timestep_stddev.cpu().detach().numpy())

np.save('Results/LSD_stddev.npy', np.array(so2_stddevs))
#np.save('Results/LSD_flowtraining_nomelanin_2_invitro_flowpredictions.npy', np.array(so2_predictions))

...
train_oxygenations = np.float32(train_oxygenations)
validation_oxygenations = np.float32(validation_oxygenations)
...
mc = tf.keras.callbacks.ModelCheckpoint('NoSkinFiltered_10.h5', monitor = 'val_median_error_fraction', mode = 'min', verbose = 1, save_best_only = True)
...
saved_model = tf.keras.models.load_model('NoSkinFiltered_10.h5', compile = False)
...
'''
reshaped_predictions = np.array(tf.reshape(predictions,[12013])) *100

reshaped_gts =np.array(tf.reshape(test_oxygenations,[12013]))*100

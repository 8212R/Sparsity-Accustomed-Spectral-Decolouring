...
train_oxygenations = train_oxygenations.numpy()
train_oxygenations = tf.convert_to_tensor(train_oxygenations)
validation_oxygenations = validation_oxygenations.numpy()
validation_oxygenations = tf.convert_to_tensor(validation_oxygenations)
test_oxygenations = test_oxygenations.numpy()
...
mc = tf.keras.callbacks.ModelCheckpoint('filteredoriginal_40.h5', monitor = 'val_median_error_fraction', mode = 'min', verbose = 1, save_best_only = True)
...
saved_model = tf.keras.models.load_model('filteredoriginal_40.h5', compile = False)
...
reshaped_predictions = np.array(tf.reshape(predictions,[14909])) *100

reshaped_gts =np.array(tf.reshape(test_oxygenations,[14909]))*100


a = create_violin_scatter_plot('lstm_skin_40.png', reshaped_predictions,reshaped_gts)

...
for i in range(timesteps):
    ...
    '''
    if i == 283:
        np.save('Time283_LSD_predictions.npy', np.array(invitro_predictions.cpu().detach().numpy()))
    timestep_so2 = torch.mean(invitro_predictions)
    timestep_std = torch.std(invitro_predictions)
    so2_predictions.append(timestep_so2.cpu().detach().numpy())
    so2_stddevs.append(timestep_std.cpu().detach().numpy())

#np.save('Results/LSD_stddev.npy', np.array(so2_stddevs))
#np.save('Results/LSD_skintraining_mediummelanin_2_invitro_flowpredictions.npy', np.array(so2_predictions))
